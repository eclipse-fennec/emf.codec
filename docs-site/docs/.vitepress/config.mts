...
// branch publishes to /emf.codec/snapshot/; tagged releases / `latest` get added
// once the first release lands.
const version = process.env.DOCS_BRANCH || 'snapshot'
const base = `/emf.codec/${version}/`
...
// pages WITHIN this version stay base-relative (e.g. `/guides/06-type`).
const SITE = 'https://eclipse-fennec.github.io/emf.codec'
...
export default defineConfig({
  title: 'Fennec Codec',
  description:
    'An EMF codec framework built on Jackson 3.x — serialize and deserialize EMF EObjects to JSON, BSON, CBOR, YAML, CSV and more.',
  lang: 'en-US',
  base,
  cleanUrls: true,
  lastUpdated: true,
  ...
  head: [
    ...
    ['meta', { property: 'og:title', content: 'Fennec Codec' }],
    [
      'meta',
      {
        property: 'og:description',
        content:
          'An EMF codec framework built on Jackson 3.x — JSON, BSON, CBOR, YAML, CSV and more.',
      },
    ],
  ],
  ...
  themeConfig: {
    ...
    socialLinks: [{ icon: 'github', link: 'https://github.com/eclipse-fennec/emf.codec' }],

    search: { provider: 'local' },

    editLink: {
      pattern: 'https://github.com/eclipse-fennec/emf.codec/edit/main/docs/:path',
