...
// (guides.mjs) — internal dev docs (development guide, plans, security analysis,
// investigations, spec-working drafts, todo/) are deliberately NOT published.
// Cross-links inside the published docs that point at a NON-published file are
// rewritten to the GitHub blob URL so they keep working instead of 404-ing on
// the site; links between published docs become sibling routes.
//
// Note: the spec lives in a sub-folder (docs/codec-v2-spec/) and its files link
// to each other with bare/relative names, so links are resolved relative to each
// source file's directory (not by bare base name) to get the right target.
...
const blobBase = `https://github.com/eclipse-fennec/emf.codec/blob/${branch}/docs`;
