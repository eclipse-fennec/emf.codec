...
- **Removed** (never read, each with a replacement): `CODEC_TRANSIENT` + `KEY_TRANSIENT` (→
  `codec.ignore`/`ignoreRead`/`ignoreWrite`, now documented in the options reference),
  `CODEC_FORMAT` (→ `typeFormat`/`idFormat`/`refFormat`; spec 17 §7.1 no longer offers a format
  switch option - the factory decides by extension/content type), `CODEC_USE_NUMERIC_IDS`
  (→ `TypeStrategy.NUMERIC`). Straight deletion, no deprecation cycle - the user's call.
- **`serialize` removed too - only `ignoreWrite`/`forceWrite` remain.** Historically `serialize`
  was a write switch that overrode the codec-level `transient` (`serialize=true` wrote a
  transient feature, today `forceWrite`); the Jan-2026 refactor left it declared but unread, and its
  javadoc even promised a mapping to `ignore`. `openapi_v3.ecore` now says `ignoreWrite=true` on
  `Operation.method` (regenerated by the user; the model fingerprint changes with it), and
  `OpenApiResourceImpl` drops its global `globalIgnore("method")`. The old OpenAPI test could
  not catch the inert annotation: it used `get`, and `GET` is the enum default that is never
  written - `methodNotSerializedForNonDefaultMethods` uses POST/PUT/DELETE.
- **`codec.inherit` stays** - rule: every annotation key has its `codec.` option pair, and they
  stay or go together. The `inherit` annotation is parsed and used by shipped models
  (blubio/dragino) but nothing acts on it; javadocs, spec 02/12/16 and the options reference now
  say so (Boolean, default `true`, no effect; type config always inherited over the full
  hierarchy, other config never). The `DIRECT`/`ALL`/`NONE` levels and the builder APIs spec 12
  described never existed and are marked as an earlier design. New `inherit="false"` case: before
  it every inherit test passed on the default value alone.
- **No guard task.** A build-time check (unit test, Gradle task, bnd analyzer) was weighed and
  dropped in favour of manual checks. Note for the next manual check: `static final String`
  constants are inlined by javac, so usage shows as the literal key in consumers, not as a field
  reference - search for both the constant name and `"codec.<key>"`, and strip comments.
- **Manual check after the removals:** of 82 `codec.` constants, only `CODEC_INHERIT` (the
  intentional pair) and `CODEC_MAX_PAYLOAD_SIZE` are neither a `ConfigProperty` nor read. The
  latter is a documented DoS limit that is never applied (BSON always uses 100 MB) → **#232**.
- **Open, noted only:** spec 17 §7.2 describes a `CodecFormatRegistry`/`CodecFormatAdapter` that
  does not exist. #222's "also turned up" annotation constants (`KEY_SUPERTYPE_WRITER_NAME`,
  `KEY_TYPE_DISCRIMINATOR_PREFIX`, `KEY_INLINE_MAPPING_PREFIX`) were not looked at.
