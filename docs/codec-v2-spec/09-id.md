...
### 4.4 Almost No ID Configuration on the Reference
...
Two properties are the exception, because they describe how the identity is *written* rather
than how it is *built* (issue #176):

| Property | Scoped to a reference means |
|---|---|
| `idKey` | the outer JSON key the identity is written under, for the objects reached through this reference |
| `idFormat` | whether that key carries the id plainly or as a nested object |

They apply to **containment** references only. A non-containment reference writes a `_ref` or a
bare URI, not the target's body, so there is no id key in that position to rename; identifying a
target *by* its id instead of by URI is a separate feature and not this one. Configured on a
non-containment reference anyway, `idKey` / `idFormat` are **dropped and reported as a WARNING** —
from an annotation at registration time and from a runtime source at resolution time alike (issue
#189). Nothing about the value is malformed, it just has nothing to act on.

```xml
<eStructuralFeatures xsi:type="ecore:EReference" name="items" eType="#//Item"
                     containment="true" upperBound="-1">
  <eAnnotations source="http://eclipse.org/fennec/codec">
    <details key="idKey" value="itemId"/>
  </eAnnotations>
</eStructuralFeatures>
<eStructuralFeatures xsi:type="ecore:EReference" name="archived" eType="#//Item"
                     containment="true" upperBound="-1"/>
```

```json
{
  "items":    [ { "itemId": "i1", "label": "Bolt" } ],
  "archived": [ { "_id":    "i2", "label": "Nut"  } ]
}
```

The same class, two references, two id keys — which is the whole point of scoping it to a
reference. The class-level `idKey` still reaches both; the reference-level one overrides it for
its own subtree, and everything else about the identity (`idKeyMode`, `idValueKey`, the
separator) continues to come from the class.

**The read side applies the same scope.** A renamed id key is expected under the reference it
was configured on, so a document written this way reads back. A class-only id property found at
the feature level is dropped **and reported** — silently ignoring it is what made this gap hard
...
**Load/save options** (`idFeatures` is class-scoped, so the id settings go into `codec.eClassConfig`):
```java
Map<String, Object> options = Map.of(
    CodecOptions.CODEC_ECLASS_CONFIG, Map.of(PersonPackage.Literals.PERSON, Map.of(
        CodecOptions.CODEC_ID_STRATEGY, "COMBINED",
        CodecOptions.CODEC_ID_FEATURES, List.of("firstName", "lastName"),
        CodecOptions.CODEC_ID_SEPARATOR, "-")));
```
...
Map<String, Object> options = Map.of(
    CodecOptions.CODEC_ECLASS_CONFIG, Map.of(PersonPackage.Literals.PERSON, Map.of(
        ...
        CodecOptions.CODEC_ID_FEATURES, List.of("firstName", "lastName"),
        CodecOptions.CODEC_ID_SEPARATOR, "-",
        CodecOptions.CODEC_ID_KEY_MODE, "BOTH",
...
Map<String, Object> options = Map.of(
    CodecOptions.CODEC_ECLASS_CONFIG, Map.of(PersonPackage.Literals.PERSON, Map.of(
        CodecOptions.CODEC_ID_STRATEGY, "COMBINED",
        CodecOptions.CODEC_ID_FEATURES, List.of("firstName", "lastName"),
        CodecOptions.CODEC_ID_KEY_MODE, "FEATURE_ONLY")));
```
...
**Load/save options:**
```java
Map<String, Object> options = Map.of(CodecOptions.CODEC_ID_KEY_MODE, "NONE");
```
...
| **Per-class config** | ✅ | ✅ | Via EAnnotation or `codec.eClassConfig` |
...
| **Direction-specific** | ❌ | ❌ | Direction is fixed per property; no `codec.ser.`/`codec.deser.` prefixes ([02 §11.7](02-config-resolution.md)) |
...
```java
Map<String, Object> options = Map.of(
    CodecOptions.CODEC_ID_STRATEGY, "ID_FIELD",
    CodecOptions.CODEC_ID_SCOPE, "ROOT_ONLY",
    CodecOptions.CODEC_ID_FORMAT, "STRUCTURED",
    CodecOptions.CODEC_ID_FORMAT_SCOPE, "ALL");
```
...
Prefix fields written by registered `CodecPrefixWriter`s ([14-custom-values.md §13](14-custom-values.md#13-prefix-readerswriters))
follow the **last** metadata field in either row and precede the features; `idOnTop` moves `_id`,
not the prefix.
