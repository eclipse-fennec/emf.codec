...
**Combined value on a derived `eID` attribute:** when the EClass carries an `eID="true"` attribute that is **not** part of `idFeatures`, it is treated as a derived key and receives the combined value (`"John-Doe-42"` in the example above). When the `eID` attribute **is** one of the `idFeatures`, it keeps its own component value — the combined value is never written over it (issue #108).
