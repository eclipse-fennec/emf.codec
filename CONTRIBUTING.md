# Contributing to Eclipse Fennec — EMF Codec
...
* This repository: https://github.com/eclipse-fennec/emf.codec
* Issue tracker: https://github.com/eclipse-fennec/emf.codec/issues
...
This repository follows the conventions documented in
[`CLAUDE.md`](CLAUDE.md). The two that matter most for contributors:

* **Spec-first.** The codec behaviour is normatively defined under
  [`docs/codec-v2-spec/`](docs/codec-v2-spec/00-overview.md). Behaviour
  changes start as a spec change, not a code change.
* **Tests with behaviour changes.** Any change to runtime behaviour ships
  with the test(s) that demonstrate the new contract. PRs without them are
...
* Search the [issue tracker](https://github.com/eclipse-fennec/emf.codec/issues)
...
7. **Wait for CI.** All status checks (build matrix on JDK 21 + 25, license
   header check, Claude PR review) must be green before review. See
...
```bash
git commit -s -m "Honor TypeHintMode.NONE in CSV serializer"
```
...
* Issues: https://github.com/eclipse-fennec/emf.codec/issues
